...
data <- ww_profiles %>%
  mutate(
    personality = case_when(
      ...
      personality == "deranged" ~ "reasonable",
      TRUE ~ personality
    ),
    rating = case_when(
      trait_max == personality ~ avg_rating,
      TRUE ~ 100 - avg_rating
    )
  ) |>
  ...
  mutate(
    image = paste0(
      "../images/",
      tolower(str_replace_all(character, " ", "_")),
      ".png"
    ),
    label = paste0(
      "<span style='color:white;font-size:10pt;'>",
      toupper(personality),
      "</span><br><span style='color:#A6ECFF;size:'10pt;'>[",
      round(rating),
      "]</span>"
    )
  )
...
line <- data.frame(
  x = rep(traits, 2),
  y = c(rep(0, length(traits)), rep(100, length(traits)))
)
...
ggplot(data, aes(y = rating, x = personality, group = character)) +
  ...
  geom_point(
    inherit.aes = FALSE,
    data = data.frame(x = traits, y = rep(100, length(traits))),
    mapping = aes(x = x, y = y),
    shape = 21,
    fill = "#1D3540",
    color = "white",
  ...
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "#1D3540", color = NA),
